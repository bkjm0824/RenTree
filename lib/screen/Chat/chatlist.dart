...
class _ChatScreenState extends State<ChatListScreen> {
    ...
    if (res.statusCode == 200) {
      ...
      for (var room in data) {
        ...
        if (raw != null) {
          ...
          room['lastMessageTime'] = parsed['sentAt']; // 👈 이것까지 추가
        } else {
          room['lastMessage'] = '메시지 없음';
          room['lastMessageTime'] = room['createdAt']; // 기본값
        }
      }
  ...
  Future<String?> getLastMessageForRoom(int chatRoomId, String type) async {
    final url = Uri.parse('http://10.0.2.2:8080/chatmessages/$type/$chatRoomId');
    final res = await http.get(url);
                  ...
                  : _chatRooms.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            print('✅ writerNickname: ${room['writerNickname']}');
                                    ...
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) => ChatRentalScreen(
                                          ...
                                          rentalItemId: room['relatedItemId'], // ✅ rental 전용
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) => ChatRequestScreen(
                                          ...
                                          userName: (_myStudentNum == room['writerStudentNum'])
                                              ? room['requesterNickname']
                                              : room['writerNickname'],
                                          title: room['itemRequestTitle'] ?? '제목 없음',
                                          requestId: room['relatedItemId'], // ✅ request 전용
                                          writerStudentNum: room['writerStudentNum'] ?? '',
                                          requesterStudentNum: room['requesterStudentNum'] ?? '',
                                          receiverStudentNum: (_myStudentNum == room['writerStudentNum'])
                                              ? room['requesterStudentNum']
